...
public class BearerOidcClient
	...
	private BearerCredentials convertTokenToCredentials(WebContext context,
			String accessToken, BearerAccessToken token)
		...
		UserInfoResponse response = parse(httpResponse);

		if (response instanceof UserInfoErrorResponse) {
			logger.error("Bad User Info response, error={}",
					((UserInfoErrorResponse) response).getErrorObject());
			throw unauthorized("", context, realmName);
		}

		OidcProfile profile = new OidcProfile(token);
		UserInfo userInfo = ((UserInfoSuccessResponse) response).getUserInfo();
		if (userInfo != null) {
